...
#	  print(dcg[i])
	if i==0 or i==1:
		...
		d = 1/math.log(i+1,2)
        ideal_ndcg = ideal_ndcg + (2**mm[i]-1)*d
        real_ndcg = real_ndcg + (2**dcg[i]-1)*d
    if ideal_ndcg ==0:
	return 1
...
path_exclu = '/aut/proj/ir/jianghong/mq2008/Fold5/'        #for group 5, only test dataset is here
#paths.remove(path_exclu+'train.txt')
feature,rank,qid,docid,leng=load_data([path_exclu+'train.txt',path_exclu+'vali.txt'])
feature_vali,rank_vali,qid_vali,docid_vali,leng_vali = load_data([path_exclu+'test.txt'])
gamma = 0.9
theta = 0.001
#print(n_classes)
#Algorithm 1
...
for ss in range(1):
    ...
    saver.restore(sess,'/aut/proj/ir/jianghong/test_l3_mq2008_k5/MyModel_1019_05absndcg60.5444386238431758')
    feed_dict=dict()
    for epoach in range(1):
...
#		print(ndcg(np.array(rank0),r_chosen,10))
                ...
                ndcg1=ndcg(np.array(rank0),y_chosen,1)
                ndcg3 = ndcg(np.array(rank0),y_chosen,3)
                ndcg5 = ndcg(np.array(rank0),y_chosen,5)
                ndcg10 = ndcg(np.array(rank0),y_chosen,10)
	        with open("/aut/proj/ir/jianghong/test_l3_mq2008_k5/ndcg1.txt","a") as f1:
                       content = str(ndcg1)+"\n"
                       f1.write(content)
		with open("/aut/proj/ir/jianghong/test_l3_mq2008_k5/ndcg3.txt","a") as f2:
                       content = str(ndcg3)+"\n"
                       f2.write(content)
		with open("/aut/proj/ir/jianghong/test_l3_mq2008_k5/ndcg5.txt","a") as f3:
                       content = str(ndcg5)+"\n"
                       f3.write(content)

		with open("/aut/proj/ir/jianghong/test_l3_mq2008_k5/ndcg10.txt","a") as f4:
                       content = str(ndcg10)+"\n"
                       f4.write(content)
